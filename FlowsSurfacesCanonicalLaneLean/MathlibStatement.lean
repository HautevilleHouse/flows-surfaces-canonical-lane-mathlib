...
Flows Surfaces closure package. The pilot closes over its admitted class and carries the
...
def theoremSpecificClosurePackageClosed : Prop :=
  ∀ A : AdmissibleClass, FlowsSurfacesConstrainedTheoremClosure A

theorem theorem_specific_closure_package_checked :
    theoremSpecificClosurePackageClosed := by
  intro A
  exact constrained_flows_surfaces_endgame A
